...
export default function PickIngredientsPage({
  ...
  const vegetablesData = ingredients.filter((i) => i.type === 'vegetable');
  const meatsData = ingredients.filter((i) => i.type === 'meat');
  const spiciesData = ingredients.filter((i) => i.type === 'spicy');
  const othersData = ingredients.filter((i) => i.type === 'other');
  ...
  return (
    <div className='flex h-full justify-between gap-9 px-2 pt-4 md:px-12 md:pt-8'>
      <div className='relative flex h-full w-full flex-col gap-6 pb-7'>
        ...
        <Tabs defaultValue='vegetable' className='h-full overflow-hidden pb-8'>
